...
import { songsReleasedIn1971 } from './OldSongApi';
import { useDispatch, useSelector } from "react-redux";
import { setCurrent } from "../../stores/player";
import { Icon } from "../../Icons";

function SeventiesSong() {
    const [oldSongs, setOldSongs] = useState([]);

    console.log("allSongs=" + oldSongs);
    const dispatch = useDispatch();
    const { current, playing, controls } = useSelector((state) => state.player);
  
    useEffect(() => {
      // Fetch all trending songs when the component mounts
      songsReleasedIn1971("1972-01-01T00:00:00.000Z").then((data) => {
        setOldSongs(data);
